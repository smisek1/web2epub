...
class conn_string:
    ...
    '''
    def __init__(self):
        self.conn = psycopg2.connect("dbname='backuptest3' user='borec' password='Passw0rd' host='localhost'")
...
class select_sites(conn_string):
    ...
    def select(self):
        cursor = self.conn.cursor()
        cursor.execute("""select id_stranka,jmeno, link, xpath_links, xpath_nadpis,xpath_clanek, xpath_datum, xpath_uvodni_odstavec, xpath_autor from stranka where id_stranka !=5 and id_stranka !=6 and (id_stranka =7 or id_stranka =2 or id_stranka =8);""")
        #cursor.fetchall()
        self.conn.commit()
...
where kniha_clanek.id_clanky is null and (stranka.id_stranka = 7 or stranka.id_stranka = 2 or stranka.id_stranka = 8) 
